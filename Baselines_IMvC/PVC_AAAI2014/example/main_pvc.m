...
for idata=1:numdata
    ...
    for v1 = 1:n_view
        for v2 = v1:n_view
            ...
            % % take truthF in case of some examples are not in all views.
            ...
            for f = 1:10 %numFold
              ...
              [Ux Uy P2 P1 P3 objValue ACC(f) NMI(f) F1(f) ARI(f)] = PVCclust(x_paired',y_paired',x_single',y_single',numClust,truthF,option);
            end
            R(v1, v2, 1) = mean(ACC);
            R(v1, v2, 2) = mean(NMI);
            Result(1,:) = ACC;
